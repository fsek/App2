...
class ProcessionScreenPage extends StatefulWidget {
  @override
  _ProcessionScreenState createState() => _ProcessionScreenState();
}



class _ProcessionScreenState extends State<ProcessionScreenPage> {
  List<Person> people = [];
  Person? _selectedPerson;
  int? _selectedPersonIndex;
  _ProcessionScreenState() {
    _loadPeople().then(
      (value) => setState(
        () {
          this.people = value;
  ...
  Future<List<Person>> _loadPeople() async {
    final String raw =
        await rootBundle.loadString('assets/data/nolleguide_2024/procession.json');
