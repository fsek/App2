...
class MinistryOfCultureScreenPage extends StatefulWidget {
  @override
  _MinistryOfCultureScreenState createState() => _MinistryOfCultureScreenState();
}



class _MinistryOfCultureScreenState extends State<MinistryOfCultureScreenPage> {
  List<Person> people = [];
  Person? _selectedPerson;
  int? _selectedPersonIndex;
  _MinistryOfCultureScreenState() {
    _loadPeople().then(
      (value) => setState(
        () {
          this.people = value;
  ...
  Future<List<Person>> _loadPeople() async {
    final String raw =
        await rootBundle.loadString('assets/data/nolleguide_2024/ministryOfCulture.json');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(t.ministryOfCulture),
      ),
      body: Container(
        child: Column(
          children: [
