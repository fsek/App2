...
class ConscienceScreenPage extends StatefulWidget {
  @override
  _ConscienceScreenState createState() => _ConscienceScreenState();
}



class _ConscienceScreenState extends State<ConscienceScreenPage> {
  List<Person> people = [];
  Person? _selectedPerson;
  int? _selectedPersonIndex;
  _ConscienceScreenState() {
    _loadPeople().then(
      (value) => setState(
        () {
          this.people = value;
  ...
  Future<List<Person>> _loadPeople() async {
    final String raw =
        await rootBundle.loadString('assets/data/nolleguide_2024/conscience.json');
