...
class EducationalCouncilScreenPage extends StatefulWidget {
  @override
  _EducationalCouncilScreenState createState() => _EducationalCouncilScreenState();
}



class _EducationalCouncilScreenState extends State<EducationalCouncilScreenPage> {
  List<Person> people = [];
  Person? _selectedPerson;
  int? _selectedPersonIndex;
  _EducationalCouncilScreenState() {
    _loadPeople().then(
      (value) => setState(
        () {
          this.people = value;
  ...
  Future<List<Person>> _loadPeople() async {
    final String raw =
        await rootBundle.loadString('assets/data/nolleguide_2024/educationalCouncil.json');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(t.educationalCouncil),
      ),
      body: Container(
        child: Column(
          children: [
