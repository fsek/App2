...
class FestivitiesCommitteeScreenPage extends StatefulWidget {
  @override
  _FestivitiesCommitteeScreenState createState() => _FestivitiesCommitteeScreenState();
}



class _FestivitiesCommitteeScreenState extends State<FestivitiesCommitteeScreenPage> {
  List<Person> people = [];
  Person? _selectedPerson;
  int? _selectedPersonIndex;
  _FestivitiesCommitteeScreenState() {
    _loadPeople().then(
      (value) => setState(
        () {
          this.people = value;
  ...
  Future<List<Person>> _loadPeople() async {
    final String raw =
        await rootBundle.loadString('assets/data/nolleguide_2024/festivitiesCommittee.json');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(t.festivitiesCommittee),
      ),
      body: Container(
        child: Column(
          children: [
