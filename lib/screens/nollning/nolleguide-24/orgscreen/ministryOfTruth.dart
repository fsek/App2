...
class MinistryOfTruthScreenPage extends StatefulWidget {
  @override
  _MinistryOfTruthScreenState createState() => _MinistryOfTruthScreenState();
}



class _MinistryOfTruthScreenState extends State<MinistryOfTruthScreenPage> {
  List<Person> people = [];
  Person? _selectedPerson;
  int? _selectedPersonIndex;
  _MinistryOfTruthScreenState() {
    _loadPeople().then(
      (value) => setState(
        () {
          this.people = value;
  ...
  Future<List<Person>> _loadPeople() async {
    final String raw =
        await rootBundle.loadString('assets/data/nolleguide_2024/ministryOfTruth.json');
