...
class CorporateRelationsScreenPage extends StatefulWidget {
  @override
  _CorporateRelationsScreenState createState() => _CorporateRelationsScreenState();
}



class _CorporateRelationsScreenState extends State<CorporateRelationsScreenPage> {
  List<Person> people = [];
  Person? _selectedPerson;
  int? _selectedPersonIndex;
  _CorporateRelationsScreenState() {
    _loadPeople().then(
      (value) => setState(
        () {
          this.people = value;
  ...
  Future<List<Person>> _loadPeople() async {
    final String raw =
        await rootBundle.loadString('assets/data/nolleguide_2024/corporateRelations.json');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(t.corporateRelations),
      ),
      body: Container(
        child: Column(
          children: [
