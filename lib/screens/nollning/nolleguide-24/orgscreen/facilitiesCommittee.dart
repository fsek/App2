...
class FacilitiesCommitteeScreenPage extends StatefulWidget {
  @override
  _FacilitiesCommitteeScreenState createState() => _FacilitiesCommitteeScreenState();
}



class _FacilitiesCommitteeScreenState extends State<FacilitiesCommitteeScreenPage> {
  List<Person> people = [];
  Person? _selectedPerson;
  int? _selectedPersonIndex;
  _FacilitiesCommitteeScreenState() {
    _loadPeople().then(
      (value) => setState(
        () {
          this.people = value;
  ...
  Future<List<Person>> _loadPeople() async {
    final String raw =
        await rootBundle.loadString('assets/data/nolleguide_2024/facilitiesCommittee.json');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(t.facilitiesCommittee),
      ),
      body: Container(
        child: Column(
          children: [
