...
class AccountingDepartmentScreenPage extends StatefulWidget {
  @override
  _AccountingDepartmentScreenState createState() => _AccountingDepartmentScreenState();
}



class _AccountingDepartmentScreenState extends State<AccountingDepartmentScreenPage> {
  List<Person> people = [];
  Person? _selectedPerson;
  int? _selectedPersonIndex;
  _AccountingDepartmentScreenState() {
    _loadPeople().then(
      (value) => setState(
        () {
          this.people = value;
  ...
  Future<List<Person>> _loadPeople() async {
    final String raw =
        await rootBundle.loadString('assets/data/nolleguide_2024/accountingDepartment.json');
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: Text(t.theAccountingDepartment),
      ),
      body: Container(
        child: Column(
          children: [
