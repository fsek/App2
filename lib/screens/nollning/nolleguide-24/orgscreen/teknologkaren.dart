...
class TeknologkarenScreenPage extends StatefulWidget {
  @override
  _TeknologkarenScreenState createState() => _TeknologkarenScreenState();
}



class _TeknologkarenScreenState extends State<TeknologkarenScreenPage> {
  List<Person> people = [];
  Person? _selectedPerson;
  int? _selectedPersonIndex;
  _TeknologkarenScreenState() {
    _loadPeople().then(
      (value) => setState(
        () {
          this.people = value;
  ...
  Future<List<Person>> _loadPeople() async {
    final String raw =
        await rootBundle.loadString('assets/data/nolleguide_2024/teknologkaren.json');
