...
class _EventPageState extends State<EventPage> {
  ...
  void initState() {
    ...
    locator<UserService>().getUser().then((value) => setState(() {
          ...
          for (int i = 0; i < (this.foodPreferences?.length ?? 0); i++) {
            ...
            this.foodPreferences![i] = foodPrefsDisplay[this.foodPreferences![i]] ?? "";
          }
        }));
              ...
              children: [
                ...
                Row(
                  children: [
                    ...
                    Text(
                      "  Anmälan öppnar: " + DateFormat("d/M").format(event!.event_signup!.opens!.toLocal()) + " " + DateFormat("jm", "sv_SE").format(event!.event_signup!.opens!.toLocal()),
                    ),
                  ],
                ),
                Row(
                  children: [
                    ...
                    Text(
                      "  Anmälan stänger: " + DateFormat("d/M").format(event!.event_signup!.closes!.toLocal()) + " " + DateFormat("jm", "sv_SE").format(event!.event_signup!.closes!.toLocal()),
                    ),
                  ],
                ),
              ],
  ...
  List<Widget> _signupDetails(String? groupName, String? userType) {
    return [
      RichText(text: TextSpan(text: "Grupp: ", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.black), children: [TextSpan(text: groupName, style: TextStyle(fontWeight: FontWeight.normal, color: Colors.black))])),
      RichText(text: TextSpan(text: "Prioritet: ", style: TextStyle(fontWeight: FontWeight.bold, color: Colors.black), children: [TextSpan(text: userType, style: TextStyle(fontWeight: FontWeight.normal, color: Colors.black))])),
              ...
              children: [
                  ...
                  children: [
                    ...
                    Text(
                      ...
                      "  " +
                          ...
                          ", " +
                          DateFormat("MMMMd", "sv_SE").format(event?.starts_at?.toLocal() ?? DateTime.now()),
                      style: TextStyle(
                        fontSize: 14,
                      ),
                    ),
                ...
                Padding(
                  ...
                  child: Column(
                    crossAxisAlignment: CrossAxisAlignment.start,
                    children: [
                      Row(children: [Text("Klädkod: "), ...?event?.dress_code?.map((dressCode) => Text(dressCode + " "))]),
                      Visibility(visible: event!.cash ?? false, child: Text("Pris: " + (event?.price?.toString() ?? "") + " kr")),
                    ],
                  ),
                ),
                const Divider(),
                  ...
                  child: Column(
                    children: [
                      Visibility(
                        ...
                        child: Row(
                          children: [Icon(Icons.attach_money_rounded), Text("  Kostar pengar")],
                        ),
                      ),
                      Visibility(
